...
def main(_argv):
    ...
    if FLAGS.media.startswith('rtsp') or FLAGS.media.isdigit() or FLAGS.media.endswith(
        ...
        for k in range(0,len(scores)):
            if(scores[k] >= 0.5):
                bboxes_filt.append(bboxes[k])
                scores_filt.append(scores[k])
                
        image = draw_bboxes(image, bboxes_filt, scores_filt, classes, names, shader)

        print('Inference Time:', ms, 'ms')
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        cv2.imwrite("./misc/result.jpg", image,[int(cv2.IMWRITE_JPEG_QUALITY), 90])
