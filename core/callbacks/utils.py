...
def local_eval(func, model, image_size, test_path, name_path, verbose):
    ...
    #tmp_model = tf.keras.Model(model.layers[0].input,model.get_layer("conv2d_20").output)
    ...
    with open(tmp_path, 'a+') as f:
        for i, path in enumerate(paths, 1):
            ...
            #[t_out1] = tmp_model.predict(images)

            #toc = time.time()

            #backbone_time.append(toc - tic)
    ...
    if verbose:
        if len(infer_time) > 5:
           ...
           # s1 = np.mean(backbone_time[5:])
        else:
            s = np.mean(infer_time)
           # s1 = np.mean(backbone_time[5:])

        print('\nInference time', s*1000, 'ms')
        #print('\nBackbone time', s1*1000, 'ms')
